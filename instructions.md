...
- SKU is the shared leaf from Alerts or Insights; use `SkuRca` (`src/components/sku-rca/`) for the fixed detail layout (collapsing header + Live/Historical body + chat footer).
- SKU Live = current metrics / issues / diagnosis; SKU Historical = same customizable widget dashboard as Insights Historical (persisted per ASIN).
